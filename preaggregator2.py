...
def task(filelist, pathin, pathout):     
    ...
    if len(job_dict[job_id]) == M:
        ...
        for i in range(M):
            ...
            np.savetxt(os.path.join(pathout,'preaggregator2_lccdecoder2_'+(job_dict[job_id])[i].partition('_')[0]+'_job'+job_id+'_'+snapshot_time+'.csv'), En_Image_Batch, delimiter=',')
    else:
        print('Not receive enough results for job '+job_id)
            
if __name__ == '__main__': ##THIS IS FOR TESTING - DO THIS
    filelist= ['score2a_preaggregator2_job1_20200424.csv'] 
