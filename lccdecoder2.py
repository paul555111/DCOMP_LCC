...
def task(filelist, pathin, pathout): 
    ...
    for j in range(M):
        np.savetxt(os.path.join(pathout,'job'+job_id+'outlccdecoder2'+str(j)+'_'+snapshot_time+'.csv'), results[j], delimiter=',')

if __name__ == '__main__': ##THIS IS FOR TESTING - DO THIS
    filelist= ['preaggregator2_lccdecoder2_score2a_job1_20200424.csv','preaggregator2_lccdecoder2_score2c_job1_20200424.csv'] 
    task(filelist,'./Agg_Results2', './Results2') 
